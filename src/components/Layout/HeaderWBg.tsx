        ...
        <div className={`flex justify-between ${oswald.variable} font-sans mb-[30px] bg-white pt-[10px] absolute top-0 z-40 w-full left-0 px-[100px]`}>
            {
                isMain ? (<div className="w-[114px] h-[33px] relative">
                    <div
                        className="w-[67px] h-[33px] left-0 top-0 absolute text-center text-black text-[27px] font-bold">{time && time.getHours() < 10 ? ("0" + time?.getHours()) : time?.getHours()}:{time && time.getMinutes() < 10 ? "0" + time?.getMinutes() : time?.getMinutes()}
                    </div>
                    <div className="w-10 h-[31px] left-[74px] top-[3px] absolute">
                        <div
                            className="left-0 top-0 absolute text-center text-black text-sm font-normal leading-[18px]">{time && weekDay[time.getDay() - 1]}
                        </div>
                        <div
                            className="w-10 h-[15px] left-0 top-[16px] absolute justify-start items-start gap-[3px] inline-flex">
                            <div className="text-center text-zinc-500 text-[10px] font-normal ">{time?.getDate()}</div>
                            <div
                                className="text-center text-zinc-500 text-[10px] font-normal ">{time && months[time.getMonth()]}</div>
                        </div>
                    </div>
                </div>) : (
            ...
            <div className="w-[116px] h-[40px] justify-start items-start gap-[18px] inline-flex">
                <div className="text-center text-neutral-800 text-[27px] font-normal">KZ</div>
                <div className="text-center text-zinc-500 text-[27px] font-normal">RU</div>
                <div className="text-center text-zinc-500 text-[27px] font-normal">EN</div>
            </div>
        </div>
    )
}
