...
import {getSchoolIdThunk, getSchoolPassportThunk, getSchoolThunk} from "@/store/thunks/school.thunk";
const MainPage = () => {
            ...
            <div className="h-[420px] relative bg-white rounded-[40px] flex justify-between items-center px-[50px]">
                <div className=" h-[100%] flex flex-col gap-[36px]  py-[50px]">
                    <div className="flex flex-col gap-[16px]">
                        <div
                            className=" text-neutral-800 text-3xl font-bold leading-[30px]">{school?.school_kz_name?.substring(school?.school_kz_name?.indexOf(" ")+1).charAt(0).toUpperCase() + "" +
                            school?.school_kz_name?.substring(school?.school_kz_name?.indexOf(" ")+2)}
                        </div>
                        <div className="w-[325px] text-pink-600 text-5xl font-bold leading-[48px]">{school?.school_kz_name?.split(" ")[0]}
                        </div>
                        <div
                ...
                <div className=" h-[100%] flex flex-col gap-[30px] py-[50px]">
                    <div
                        className="w-[311px] h-[18px] text-neutral-800 text-3xl font-bold leading-[8.40px]">Школа
                        у вас в телефоне
                    </div>
                    <div className="w-[142.86px] h-[140px] relative">
                        <img src="/images/qr.svg" className="w-[163.27px] h-40 left-0 top-[-10px] absolute"/>
                    </div>
                    <div
                        className="w-[290px] text-zinc-500 text-2xl font-normal leading-[34.40px]">Отсканируйте
                        QR-код, чтобы просмотреть расписание занятий через смартфон.
                    </div>
                </div>
            </div>
            <div
                ...
                onClick={() => {router.push(`/school/${id}/scheduleTabs`);
                }}
            >
                    <div
                        className="h-[44px] text-white text-7xl font-bold leading-[30%] text-center">Расписание
                    </div>
                </div>
                ...
                {sidebar.map((item) => (
                    ...
                    >
                        <div
                            ...
                            <div className="h-[10px] text-center text-indigo-800 text-4xl font-bold leading-[27%] tracking-normal">{item.type}
                            </div>
                        </div>
                    </Link>
                ))}
